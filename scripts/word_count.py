#!/usr/bin/env python3
"""Count words in a pandoc Markdown manuscript (abstract and main text).

Usage:
    python word_count.py --paper-dir /path/to/paper
    python word_count.py --paper-dir /path/to/paper --json
    python word_count.py /path/to/manuscript.md
"""
...
def count_manuscript(path: Path) -> dict[str, int | dict[str, int]]:
    ...
    body_parts = [sections[name] for name in BODY_SECTIONS if name in sections]
    body_text = "\n\n".join(body_parts)

    by_section = {name: count_words(sections[name]) for name in BODY_SECTIONS if name in sections}
